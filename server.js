  ...
  try {
    ...
    if (url === '/api/consultancies' && req.method === 'POST') {
      ...
      await pool.query(
        ...
        [id, c.nombre, c.empresa, c.depto, c.fecha, c.consultora, c.data || {}, c.archivos || []]
      );
    ...
    if (getMatch && req.method === 'PUT') {
      ...
      await pool.query(
        `UPDATE consultancies SET nombre=$2, empresa=$3, depto=$4, fecha=$5, consultora=$6, data=$7, archivos=$8, report=$9, updated_at=NOW()
         WHERE id=$1`,
        [getMatch[1], c.nombre, c.empresa, c.depto, c.fecha, c.consultora, c.data || {}, c.archivos || [], c.report || null]
      );
