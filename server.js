...
async function handleAPI(req, res, url) {
  ...
  if (url.split('?')[0] === '/api/calcom-webhook' && req.method === 'POST') {
    try {
      ...
      // Extraer campos personalizados: empresa, proyecto, motivo, teléfono...
      function pickField(obj, keys){
        for (const k of keys) {
          if (obj[k] !== undefined) {
            const v = obj[k];
            return typeof v === 'object' ? (v.value || v.answer || '') : v;
          }
        }
        return '';
      }
      const company = pickField(responses, ['empresa','company','organization','organización']) || attendee.company || '';
      const projectField = pickField(responses, ['proyecto','project']);
      const motivo = pickField(responses, ['motivo','reason','motivo_reunion','motivo-reunion','motivoDeLaReunion']);
      const phone = pickField(responses, ['telefono','teléfono','phone','tel']);
