...
async function handleAPI(req, res, url) {
  ...
  if (url.split('?')[0] === '/api/calcom-webhook' && req.method === 'POST') {
    try {
      ...
      // Eventos típicos: BOOKING_CREATED, BOOKING_RESCHEDULED, BOOKING_CANCELLED
      const trigger = payload.triggerEvent || payload.event || 'BOOKING_CREATED';
      const p = payload.payload || payload;

      // Extraer datos comunes
      const externalId = String(p.uid || p.id || p.bookingId || ('cal_' + Date.now()));
      const attendee = (Array.isArray(p.attendees) && p.attendees[0]) || p.attendee || {};
      const attendeeName = attendee.name || p.name || p.responses?.name?.value || p.responses?.name || '';
      const attendeeEmail = attendee.email || p.email || p.responses?.email?.value || p.responses?.email || '';
      ...
      const eventType = p.eventType?.title || p.eventType?.slug || p.type || '';
      const location = p.location || (p.eventType && p.eventType.locations && p.eventType.locations[0]) || '';
      const meetingUrl = p.metadata?.videoCallUrl || p.meetingUrl || p.location || '';
      const responses = p.responses || p.customInputs || {};

      // ── Extractor tolerante: intenta múltiples formas de acceder a un campo ──
      // Cal.com puede enviar responses como:
      //   { proyecto: "Magic" }                          → simple
      //   { proyecto: { value: "Magic", label: "..." } } → objeto con value
      //   { proyecto: { value: ["Magic"] } }             → array
      //   [{ label: "proyecto", value: "Magic" }, ...]   → array de objetos
      //   { "question-1": "Magic" }                      → clave técnica
      function normalizeStr(s){ return String(s || '').toLowerCase().replace(/[\s_\-\.]/g,'').replace(/[áàä]/g,'a').replace(/[éèë]/g,'e').replace(/[íìï]/g,'i').replace(/[óòö]/g,'o').replace(/[úùü]/g,'u').replace(/ñ/g,'n'); }
      function extractValue(v){
        if (v === null || v === undefined) return '';
        if (typeof v === 'string' || typeof v === 'number') return String(v);
        if (Array.isArray(v)) return v.map(extractValue).filter(Boolean).join(', ');
        if (typeof v === 'object') {
          if (v.value !== undefined) return extractValue(v.value);
          if (v.answer !== undefined) return extractValue(v.answer);
          if (v.label !== undefined) return extractValue(v.label);
          return JSON.stringify(v).substring(0, 500);
        }
        return String(v);
      }
      // Aplana las respuestas a un mapa {key_normalizado: valor}
      function flattenResponses(obj){
        const out = {};
        if (!obj) return out;
        if (Array.isArray(obj)) {
          ...
          obj.forEach((item, idx) => {
            if (item && typeof item === 'object') {
              ...
              out[normalizeStr(key)] = extractValue(item.value !== undefined ? item.value : item);
              // También guardamos por el label original
              out['__orig__' + key] = extractValue(item.value !== undefined ? item.value : item);
            }
          });
        } else if (typeof obj === 'object') {
          Object.keys(obj).forEach(k => {
            const val = extractValue(obj[k]);
            out[normalizeStr(k)] = val;
            out['__orig__' + k] = val;
          });
        }
        return out;
      ...
      // También intentar con userFieldsResponses (v2) por si acaso
      Object.assign(flat, flattenResponses(p.userFieldsResponses));
      Object.assign(flat, flattenResponses(p.bookingFieldsResponses));
      ...
      const motivo = pickField(['motivo','reason','motivoreunion','motivodelareunion','razon','razón','porque','tema','asunto']);
      const phone = pickField(['telefono','teléfono','phone','tel','movil','móvil','contacto']);

      // Guardamos todas las respuestas visibles del formulario (con sus labels originales)
      // Extraemos las que empiezan por __orig__ que son las originales
      const allFormFields = {};
      Object.keys(flat).forEach(k => {
        if (k.startsWith('__orig__')) {
          const origKey = k.substring(8);
          const val = flat[k];
          // Excluir campos ya extraídos y campos vacíos
          if (val && String(val).trim()) allFormFields[origKey] = val;
        }
      });

      const duration = (startsAt && endsAt) ? Math.round((new Date(endsAt).getTime() - new Date(startsAt).getTime()) / 60000) : null;
      ...
      // Determinar status según trigger
      ...
      const existing = await pool.query('SELECT id FROM bookings WHERE external_id = $1', [externalId]);
      if (existing.rows.length > 0) {
        ...
        await pool.query(
          ...
          [eventType, title, attendeeName, attendeeEmail, company, startsAt, endsAt, duration, location, meetingUrl,
           status, JSON.stringify(formData), JSON.stringify(payload), externalId]
        );
      } else {
        ...
        await pool.query(
          ...
          [newId, externalId, eventType, title, attendeeName, attendeeEmail, company, startsAt, endsAt, duration,
           location, meetingUrl, status, JSON.stringify(formData), JSON.stringify(payload)]
        );
      }
