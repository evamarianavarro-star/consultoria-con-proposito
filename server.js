...
function transformReportHtml(html) {
  ...
  // 1) Sanear contenido problemático para XML/DOCX

  // Eliminar caracteres de control inválidos en XML (excepto tab, LF y CR)
  out = out.replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\uFFFE\uFFFF]/g, '');

  // Eliminar comentarios HTML que html-to-docx no maneja bien
  out = out.replace(/<!--[\s\S]*?-->/g, '');

  // Quitar etiquetas problemáticas o no soportadas (style, script, etc.)
  out = out.replace(/<style[\s\S]*?<\/style>/gi, '');
  out = out.replace(/<script[\s\S]*?<\/script>/gi, '');

  // Quitar atributos con nombres extraños tipo @something o data-something
  // (estos provocan "Invalid XML name")
  out = out.replace(/\s+@[a-zA-Z][\w-]*\s*=\s*"[^"]*"/g, '');
  out = out.replace(/\s+@[a-zA-Z][\w-]*\s*=\s*'[^']*'/g, '');
  out = out.replace(/\s+data-[\w-]+\s*=\s*"[^"]*"/g, '');
  out = out.replace(/\s+data-[\w-]+\s*=\s*'[^']*'/g, '');

  // Quitar atributos con espacios o caracteres especiales en el nombre (que rompen XML)
  out = out.replace(/\s+[a-zA-Z][\w-]*:[\w-]+\s*=\s*"[^"]*"/g, '');

  // Eliminar &nbsp; y similares que a veces dan problemas, reemplazándolos por espacio normal
  out = out.replace(/&nbsp;/g, ' ');

  // 2) Aplicar estilos inline

  // <h1> (por si Claude lo usa) → estilo grande
  out = out.replace(/<h1([^>]*)>/gi, '<h1 style="font-family: Calibri, sans-serif; font-size:20pt; font-weight:600; color:#1A1814; margin:28pt 0 12pt 0;"$1>');
  // <h2> → estilo medio
  out = out.replace(/<h2([^>]*)>/gi, '<h2 style="font-family: Calibri, sans-serif; font-size:16pt; font-weight:600; color:#1A1814; margin:24pt 0 10pt 0;"$1>');
  // <h3> → mayúsculas grises
  out = out.replace(/<h3([^>]*)>/gi, '<h3 style="font-family: Calibri, sans-serif; font-size:9pt; font-weight:600; color:#6B6560; text-transform:uppercase; letter-spacing:1.5pt; margin:16pt 0 6pt 0;"$1>');
  // <p>
  out = out.replace(/<p([^>]*)>/gi, '<p style="font-family: Calibri, sans-serif; font-size:11pt; line-height:1.6; margin:0 0 10pt 0;"$1>');
  // <ul>
  out = out.replace(/<ul([^>]*)>/gi, '<ul style="margin:6pt 0 12pt 0; padding-left:20pt;"$1>');
  // <ol>
  out = out.replace(/<ol([^>]*)>/gi, '<ol style="margin:6pt 0 12pt 0; padding-left:20pt;"$1>');
  // <li>
  out = out.replace(/<li([^>]*)>/gi, '<li style="font-family: Calibri, sans-serif; font-size:11pt; line-height:1.55; margin-bottom:4pt;"$1>');
  // <strong> y <b>
  out = out.replace(/<strong([^>]*)>/gi, '<strong style="font-weight:600;"$1>');
  out = out.replace(/<b([^>]*)>/gi, '<b style="font-weight:600;"$1>');
...
async function handleAPI(req, res, url) {
  ...
  try {
    ...
    if (reportDocxMatch && req.method === 'GET') {
      ...
      // HTML con estilos minimalistas tipo Montserrat (Word lo aproximará a Calibri si Montserrat no existe)
      const fullHtml = `
        <!DOCTYPE html>
        <html>
        <head><meta charset="UTF-8"></head>
        <body style="font-family: Calibri, sans-serif; color: #1A1814;">
          <p style="font-size:9pt;letter-spacing:2pt;color:#6B6560;text-transform:uppercase;margin:0 0 24pt 0;">${escapeHtml(typeLabel)}</p>
          <h1 style="font-family: Calibri, sans-serif; font-size:28pt; font-weight:300; color:#1A1814; margin:0 0 20pt 0; line-height:1.15;">${escapeHtml(rep.title)}</h1>
          <table style="width:100%; border-top:1px solid #E2DDD5; border-collapse:collapse; margin-bottom:24pt;">
            <tr>
              <td style="padding:12pt 12pt 0 0; width:25%;">
                <div style="font-size:8pt; letter-spacing:1.5pt; color:#A09A92; text-transform:uppercase; margin-bottom:4pt;">Cliente</div>
                <div style="font-size:11pt; font-weight:500;">${escapeHtml(rep.empresa || rep.consultancy_name || '—')}</div>
              </td>
              ${rep.depto ? `<td style="padding:12pt 12pt 0 0; width:25%;">
                <div style="font-size:8pt; letter-spacing:1.5pt; color:#A09A92; text-transform:uppercase; margin-bottom:4pt;">Departamento</div>
                <div style="font-size:11pt; font-weight:500;">${escapeHtml(rep.depto)}</div>
              </td>` : ''}
              <td style="padding:12pt 12pt 0 0; width:25%;">
                <div style="font-size:8pt; letter-spacing:1.5pt; color:#A09A92; text-transform:uppercase; margin-bottom:4pt;">Fecha</div>
                <div style="font-size:11pt; font-weight:500;">${escapeHtml(rep.fecha || new Date(rep.created_at).toLocaleDateString('es-ES'))}</div>
              </td>
              ${rep.consultora ? `<td style="padding:12pt 0 0 0; width:25%;">
                <div style="font-size:8pt; letter-spacing:1.5pt; color:#A09A92; text-transform:uppercase; margin-bottom:4pt;">Consultora</div>
                <div style="font-size:11pt; font-weight:500;">${escapeHtml(rep.consultora)}</div>
              </td>` : ''}
            </tr>
          </table>
