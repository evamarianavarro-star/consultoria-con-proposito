...
Solo HTML puro, sin markdown. Cada sección en <div class="report-section"> con <h2>. Sin DOCTYPE, sin <html>, sin bloques \`\`\`. Empieza directamente con <div class="report-section"><h2>Acta...`;
      ...
      return sendJSON(res, 200, { report: clean, title: 'Acta — ' + (inter.title || typeLabel) + ' (' + (inter.date || '') + ')' });
    }
...
HTML puro. Cada sección en <div class="report-section"> con <h2>. Sin markdown, sin bloques \`\`\`, sin DOCTYPE. Empieza directamente con <div class="report-section">.`;
      ...
      let clean = result.replace(/```html\s*/gi,'').replace(/```\s*$/g,'').trim();
      return sendJSON(res, 200, { report: clean, title: 'Seguimiento — ' + proj.title });
    }
...
- title: título breve y claro (máx. 80 caracteres)
- description: descripción concisa (2-3 frases) de qué se quiere conseguir
- priority: "high" | "medium" | "low" (según la urgencia/impacto que veas en el informe)
- responsible_persona_id: si un responsable natural aparece claro en el informe, pon su id (de la lista arriba). Si no, deja "".
- rationale: breve explicación (1 frase) de por qué has propuesto este proyecto y de qué parte del informe sale

--- REGLAS ---
- NO inventes propuestas que no estén en los informes. Solo lo que esté escrito.
- Agrupa cosas relacionadas en un mismo proyecto (no fragmentes en 10 fichas casi iguales)
- Prioriza calidad sobre cantidad: mejor 4-6 proyectos bien definidos que 15 vagos
- Si una propuesta ya está muy detallada, respeta esa granularidad
- Los títulos deben ser accionables (empieza con verbo cuando sea posible)

--- FORMATO DE SALIDA (OBLIGATORIO) ---
Devuelve SOLO un array JSON válido, sin texto extra, sin bloques de código, sin explicaciones. Estructura exacta:
[
  {"title":"...","description":"...","priority":"high","responsible_persona_id":"","rationale":"..."},
  ...
]`;

      const contentBlocks = [{ type: 'text', text: prompt }];
      const result = await callAnthropic([{ role: 'user', content: contentBlocks }], 6000);
      let clean = result.replace(/```json\s*/gi,'').replace(/```\s*/g,'').trim();
      let projects = [];
      try {
        // Buscar el primer [ y el último ] para robustez
        const start = clean.indexOf('[');
        const end = clean.lastIndexOf(']');
        if (start >= 0 && end > start) clean = clean.substring(start, end+1);
        projects = JSON.parse(clean);
      } catch (e) {
        return sendJSON(res, 500, { error: 'La IA no devolvió un JSON válido. Reintenta.' });
      }
