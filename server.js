...
${transformReportHtml(rep.content)}
    ...
    if (projListMatch && req.method === 'POST') {
      ...
      await pool.query(
        `INSERT INTO projects (id, consultancy_id, title, description, status, priority, responsible_persona_id, target_date, progress, notes, sort_order)
    ...
    if (projItemMatch && req.method === 'PUT') {
      ...
      await pool.query(
        `UPDATE projects SET title=$1, description=$2, status=$3, priority=$4, responsible_persona_id=$5,
                target_date=$6, progress=$7, notes=$8, sort_order=$9, updated_at=NOW()
         WHERE id=$10`,
        [p.title, p.description || null, p.status, p.priority, p.responsible_persona_id || null,
         p.target_date || null, p.progress || 0, p.notes || null, p.sort_order || 0, projItemMatch[1]]
      );
      return sendJSON(res, 200, { ok: true });
    }
