...
${transformReportHtml(rep.content)}
    ...
    if (notesDocxMatch && req.method === 'GET') {
      ...
      const entrevistasConDatos = entrevistas.filter(p => {
        if (!p.resp) return false;
        return Object.keys(p.resp).some(k => hasValue(p.resp[k]));
      });
      if (entrevistasConDatos.length > 0) {
        ...
        for (const p of entrevistasConDatos) {
          ...
          for (const k of Object.keys(entrevistaLabels)) {
            ...
            body += `<p style="font-family: Calibri, sans-serif; font-size:9pt; font-weight:600; color:#6B6560; text-transform:uppercase; letter-spacing:1.5pt; margin:12pt 0 4pt 0;">${escapeHtml(entrevistaLabels[k])}</p>`;
